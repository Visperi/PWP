...
An instance of pre-populated database can be found from `instance/database.db`. This database contains random generated 
data between all allowed column values in database tables. A new pre-populated database can be generated by simply 
running module `populate_database.py`. This script does nothing else than creates the database and populates its 
tables with some dummy values.
